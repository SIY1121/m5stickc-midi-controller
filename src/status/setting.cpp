...
StatusNo SettingStatus::exec(bool isFirstPaint) {
  ...
  if(!edit) {
    ...
    if(roll > 10 && millis() - lastTime > 500) {
      selectedIndex = min(3, selectedIndex + 1);
      lastTime = millis();
    } else if(roll < -10 && millis() - lastTime > 500) {
      selectedIndex = max(0, selectedIndex - 1);
      lastTime = millis();
    }
    for(int i = 0; i < 4; i++){
      M5.Lcd.setTextColor(i == selectedIndex ? TFT_GREENYELLOW : TFT_WHITE, TFT_DARKGREY);
